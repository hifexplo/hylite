"""
sklearn-compatible dimensionality reduction for hyperspectral data (PCA, MNF).
"""
from hylite._deps import require_on_load, require
require_on_load("transform.reduction", "sklearn")
...
class MNF(BaseEstimator, TransformerMixin):
    ...
    perform MNF transforms, and (2) works with either `hylite.hydata.HyData` instances or numpy arrays.
    """
    ...
    def transform(self, X):
        """Apply PCA band-space transformation, returning a `hylite.hydata.HyData` instance or numpy array matching X."""
        out = None
        if isinstance(X, hylite.HyData):
    ...
    def inverse_transform(self, Xt):
        """Inverse MNF or PCA to reconstruct cube, returning a `hylite.hydata.HyData` instance or numpy array matching Xt."""
        out = None
